...
class LeafNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        while (cur != null) {
            ...
            // key > last
            if (!cur.rightSibling.isPresent()) {
                // no tail anymore, it must be in this node theoretically event
                // it's not in it, actually.
                break;
            }
    ...
    public LeafNode getLeftmostLeaf() {
        // TODO(proj2): implement
        // the left most node of a leaf node is itself.
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (this.keys.size() > 2 * order) {
            ...
            List<RecordId> rightRids = this.rids.subList(order, this.rids.size());
            ArrayList<DataBox> boxes = new ArrayList<>(2 * order);
            boxes.addAll(rightKeys);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long sib = buf.getLong();
        int count = buf.getInt();

        ArrayList<DataBox> boxes = new ArrayList<>(count);
        ArrayList<RecordId> records = new ArrayList<>(count);

        for (int i = 0; i < count; i++) {
            boxes.add(DataBox.fromBytes(buf, metadata.getKeySchema()));
            records.add(RecordId.fromBytes(buf));
        }

        return new LeafNode(metadata, bufferManager, page, boxes, records,
                sib == -1 ? Optional.empty() : Optional.of(sib), treeContext);
    }
